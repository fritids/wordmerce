		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'text_time':
					...
					break;					
				case 'text_money':
					echo '&pound; <input class="cmb_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'colorpicker':
				...
				case 'textarea':
					...
					if(isset($field['limit'])){
						echo 'You have <span id="charsLeft"></span> chars left.';
						echo '
							<script type="text/javascript">
							    jQuery(\'#', $field['id'], '\').limit(\'', $field['limit'], '\',\'#charsLeft\');
							</script>
						';
					}
					break;
				...
				break;
				case 'map_picker':
				echo '<input class="map_picker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />','<p class="cmb_metabox_description">', $field['desc'], '</p>';
				break;
				case 'password':
					echo '
						<script>
						jQuery(function(){
						  jQuery("#'.$field['id'].'show").click(function(){
						    if( jQuery("#'.$field['id'].'show:checked").length > 0 ){
						      var pswd = jQuery("#'.$field['id'].'").val();
						      jQuery("#'.$field['id'].'").attr("id","'.$field['id'].'2");
						      jQuery("#'.$field['id'].'2").after( jQuery("<input name=\''.$field['id'].'\' id=\''.$field['id'].'\' type=\'text\'>") );
						      jQuery("#'.$field['id'].'2").remove();
						      jQuery("#'.$field['id'].'").val( pswd );
						    }
						    else{
						      var pswd = jQuery("#'.$field['id'].'").val();
						      jQuery("#'.$field['id'].'").attr("id","'.$field['id'].'2");
						      jQuery("#'.$field['id'].'2").after( jQuery("<input name=\''.$field['id'].'\' id=\''.$field['id'].'\' type=\'password\'>") );
						      jQuery("#'.$field['id'].'2").remove();
						      jQuery("#'.$field['id'].'").val( pswd );
						    }
						  });
					...
					';
					echo '<input type="password" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />','<input id="'.$field['id'].'show" type="checkbox" /><label for="'.$field['id'].'show">show password</label><p class="cmb_metabox_description">Type a new password above and click update.</p><p class="cmb_metabox_description">', $field['desc'], '</p>';
					//echo get_post_meta( $post->ID, $field['id'].'_hash', true);
				break;
				case 'function' :
					call_user_func($field['function']);
				break;
				default:
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( $field['type'] == 'password' ) {
				if($new != ''){
				    ...
				    $new_pass = $wp_hasher->HashPassword($new);
				    
				    update_post_meta($post_id, $field['id'].'_hash', $new_pass);
			...
			if ( isset( $field['validate_func']) ) {
				$ok = call_user_func( $field['validate_func'], $new );
...
wp_register_script( 'cmb-timepicker', CMB_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		...
		wp_enqueue_script( 'cmb-timepicker' , array('google-maps'));
