...
if( !class_exists( 'acf_image_picker' ) && class_exists( 'acf_Field' ) ) :

class acf_image_picker extends acf_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function __construct($parent)
	{
		...
		$this->title = __("Image Picker",'acf');
		...
		add_filter('acf_save_field-image_picker', array($this, 'acf_save_field'));
   	
		add_action('acf_head-input', array($this, 'acf_head_update_attachment'));
   	...
   	function acf_head_update_attachment(){
	   	...
	   	wp_enqueue_script('image-picker', $paths['url'].$paths['fcw_path'].'/modules/wordpress/custom_fields/js/image-picker.js');
	   	
	   	wp_enqueue_style('image-picker', $paths['url'].$paths['fcw_path'].'/modules/wordpress/custom_fields/css/image-picker.css');
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		// html
		echo '<select id="' . $field['id'] . '" class="' . $field['class'] . ' image_picker" name="' . $field['name'] . '" ' . $multiple . ' >';	
		...
		foreach($field['choices'] as $key => $value)
		{
			if($field['optgroup'])
			{
				...
				if($value)
				{
					...
					{
						$selected = '';
						if(is_array($field['value']) && in_array($label, $field['value']))
						{
						...
						echo '<option data-img-src="'.$label.'" value="'.$label.'" '.$selected.'>'.$label.'</option>';
					}
				}
			...
			else
			{
				...
				echo '<option data-img-src="'.$value.'" value="'.$value.'" '.$selected.'>'.$value.'</option>';
			}
